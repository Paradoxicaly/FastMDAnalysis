...
def _plot_summary(summary: dict[str, dict], runs: dict[str, list[RunMetrics]], output_dir: Path) -> None:
    ...
    plot_mem_display = [max(total - calc, 0.0) for calc, total in zip(calc_mem_means, total_mem_means)]
    fig, ax = plt.subplots(figsize=(8, 5))
    ...
    ax.set_ylim(0, _headroom(total_mem_means, total_mem_err))
    target_bars = plot_mem_bars if has_plot_mem and plot_mem_bars is not None else calc_mem_bars
    _annotate_small_mem(ax, target_bars, total_mem_means)
    fig.tight_layout()
