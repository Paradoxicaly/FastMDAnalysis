...
OUTPUT_ROOT = PROJECT_ROOT / "benchmarks" / "results" / "rg_trpcage"
...
def _run_mdtraj(traj, rep_dir: Path) -> np.ndarray:
    ...
    _save_plot(values, plot_path, "Radius of Gyration vs Frame (MDTraj)")
    INSTRUMENT.record_files("mdtraj", [data_path, plot_path])
    return values
...
def _run_mdanalysis(universe, rep_dir: Path) -> np.ndarray:
    ...
    plot_path = rep_dir / "rg.png"
    np.savetxt(data_path, values.reshape(-1, 1), header="rg", fmt="%.6f")
    _save_plot(values, plot_path, "Radius of Gyration vs Frame (MDAnalysis)")
...
def _plot_summary(summary: dict[str, dict], runs: dict[str, list[RunMetrics]], output_dir: Path) -> None:
    ...
    ax.set_title("RG Runtime Comparison (Trp-cage)")
    ...
    ax.set_title("RG Peak Memory Comparison (Trp-cage)")
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    ...
    ax.set_title("RG Runtime Distribution (Trp-cage)")
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
