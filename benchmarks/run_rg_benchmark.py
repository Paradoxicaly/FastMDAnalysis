...
SNIPPET_ROOT = PROJECT_ROOT / "benchmarks" / "snippets" / "rg"
SNIPPET_FILES = {
...
def _set_dataset(slug: str) -> None:
    ...
    OUTPUT_ROOT = PROJECT_ROOT / "benchmarks" / "results" / f"rg_{DATASET_SLUG}"
...
def _save_plot(values: np.ndarray, out_path: Path, title: str) -> None:
    ...
    ax.plot(frames, values.flatten(), marker="o", linestyle="-")
    ax.set_title(title)
    ax.set_xlabel("Frame")
...
def _run_fastmdanalysis(
    ...
    *,
    ...
    analysis = fast_rg.RGAnalysis(traj, atoms=ATOM_SELECTION, output=str(rep_dir))
    ...
    values = analysis.data.reshape(-1).astype(float)
    ...
    if not timing_only:
        ...
        data_path = rep_dir / "rg.dat"
        np.savetxt(data_path, values.reshape(-1, 1), header="rg", fmt="%.6f")
        generated.append(data_path)
        if do_plot:
            plot_path = rep_dir / "rg.png"
            _save_plot(values, plot_path, f"Radius of Gyration vs Frame ({dataset_label})")
...
def _run_mdanalysis(
    ...
    *,
    ...
    for ts in universe.trajectory:
        ...
        values_list.append(np.sqrt((squared * masses).sum() / total_mass))
    calc_time = time.perf_counter() - start_calc
...
def _plot_summary(summary: dict[str, dict], runs: dict[str, list[RunMetrics]], output_dir: Path) -> None:
    ...
    def _annotate_mem_segments(ax, bars, values: list[float], *, threshold: float = 1.0) -> None:
    ...
    ax.set_title(f"RG Runtime Comparison ({DATASET_LABEL})")
    ...
    ax.set_xticklabels(labels)
    ax.set_ylabel("Mean Runtime (s)")
    ax.set_title(f"RG Runtime Breakdown (All Tools) ({DATASET_LABEL})")
    ax.grid(axis="y", alpha=0.3)
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run radius of gyration benchmark across toolkits.")
